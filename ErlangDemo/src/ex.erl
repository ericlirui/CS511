...
%%    {ok,R} -> if
%%                R#inh.children =:= [] -> [] ;
%%                true -> lists:flatten(lists:map(fun(X)-> descendantsOf(X,TownR)
%%                                                end,R#inh.children),R#inh.children)
%%
%%                end;
%%    error -> error
%%  end.
...
get_ancestor(Id,TownR)->
  DD = maps:filter(fun(K,V) -> lists:member(Id,V#inh.children) end,TownR),
  maps:keys(DD).
ancestorsOf(Id,TownR)->
  ancestorsOf([],TownR,[Id]).
ancestorsOf(Visited,_TownR,[])->
  Visited;
ancestorsOf(Visited,TownR,Current) ->
  DD = lists:flatten(lists:map(fun (Id) -> get_ancestor(Id,TownR) end, Current)),
  ancestorsOf(Visited++Current,TownR,DD).


my_append([H|T], Tail) when is_list(H) ->
  my_append(H, my_append(T, Tail));
my_append([L1|L2], TAIL) ->
  [ L1| my_append(L2,TAIL)];
my_append([],[]) -> [];
my_append([],E)->
  E.
