...
do_leave(State, Ref, ChatName) ->
    ...
    case maps:find(ChatName,State#cl_st.con_ch) of
      ...
      {ok,_} ->
        ...
        receive
          { _ , Ref, ack_leave} ->
           M = maps:remove(ChatName,State#cl_st.con_ch),
          {ok, next_state(State#cl_st.gui,State#cl_st.nick,M)}
      end
    end.
    ...
    true ->
      ...
      receive
        {_,Ref, err_nick_used} ->
          {err_nick_used,State};
        {_,Ref, ok_nick}->
          {ok_nick,next_state(State#cl_st.gui,NewNick,State#cl_st.con_ch)}
      end
  end.
